...
11. **Le pouvoir de switch n'est donné qu'aux actions initiées par l'utilisateur** — testé par Impre : le ctx du shortcut (ctrl+h) a , pas celui du . Design final assumé : au lancement home, le champ est prérempli  (le bouton power du hub) → un Enter → le ctx commande est vivant et TOUT switch direct ensuite. / existent dans l'API UI, aucun submit programmatique.
